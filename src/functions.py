...
def get_printable_piece(piece: PlayerPiece, color: Color, valid: bool):
    ...
    for i in range(len(piece.shape)):
        result.append(
            " ".join(
                [
                    " " if y == 0 else color.value if valid else "X"
                    for y in piece.shape[i]
                ]
            )
        )
    return result
...
def pieces(data):
    ...
    for i in range(len(player.pieces)):
        ...
        printable_piece = get_printable_piece(
            player.pieces[i], player.color, data["valid_moves"][i]
        )
...
def piece_left_swap(data):
    ...
    offset = state.board.out_of_bounds_offset(
        player.pieces[data["index"]].split(data["position"])
    )
    data["position"] = (
        data["position"][0] - offset[0],
        data["position"][1] - offset[1],
