...
class Board:
    ...
    def __init__(self, grid_items: list[list[GridItem]] = None):
        if grid_items is None:
            grid_items = [
                [None for j in range(Board.grid_size)] for i in range(Board.grid_size)
            ]
    ...
    def print_mock(self, piece_shards: list[PieceShard], color: Color):
        ...
        mock = [
            ["." if y is None else y.owner_color.value for y in x]
    ...
    def out_of_bounds(self, piece_shards: list[PieceShard]) -> bool:
        for shard in piece_shards:
            ...
            if (
                i < 0
                or j < 0
                or i >= len(self.grid_items)
                or j >= len(self.grid_items[0])
            ):
                return True
        return False
    ...
    def position_out_of_bounds(self, i: int, j: int) -> bool:
        return (
            i < 0 or j < 0 or i >= len(self.grid_items) or j >= len(self.grid_items[0])
        )

    def unit_is_color(self, i: int, j: int, color: Color):
        return (
            not self.position_out_of_bounds(i, j)
            and self.grid_items[i][j] is not None
            and self.grid_items[i][j].owner_color == color
        )
    ...
    def validate(self, piece_shards: list[PieceShard], color: Color) -> bool:
        ...
        for shard in piece_shards:
            ...
            if is_first_move and (
                ...
                is_corner = True
            if not is_first_move and (
                self.unit_is_color(i + 1, j, color)
                ...
                break
            if not is_first_move and (
                self.unit_is_color(i + 1, j + 1, color)
        ...
        return (
            is_first_move
            and is_corner
            or not is_first_move
            and touches_corner
            and not touches_side
        ) and not intersects
    ...
    def find_possible_corners(self, player: Player):
        ...
        for row in range(Board.grid_size):
            for col in range(Board.grid_size):
                ...
                same_color_side = (
                    self.unit_is_color(row + 1, col, player.color)
    ...
    def any_valid_move_piece(self, piece: PlayerPiece, color: Color, possible_corners):
        for a in range(2):
            for b in range(4):
